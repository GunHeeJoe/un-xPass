...
from .renewal_soccermap import SoccerMap, pixel, soccermap_ToSoccerMapTensor
...
class PassValueComponent(UnxpassComponent):
    ...
    def _get_metrics(self, y, y_hat):
        ...
        bce_l2 = BinaryCalibrationError(n_bins=10,norm='l2')
        bce_max = BinaryCalibrationError(n_bins=10,norm='max')
        ...
        _y = y
        # if "scores" in y:
        #     _y = y.reset_index()[["scores"]]
        # elif "concedes" in y:
        #     _y = y.reset_index()[["concedes"]]
        # else:
        #     return {}
        ...
        return {
            ...
            "bce_l2" : bce_l2(tensor_y_hat, tensor_y),
            "bce_max" : bce_max(tensor_y_hat, tensor_y)
        }
...
class PytorchSoccerMapModel(pl.LightningModule):
    ...
    def __init__(
        ...
        self.model = SoccerMap(in_channels=15)
    ...
    def training_step(self, batch: Any, batch_idx: int):
        ...
        # log train metrics
        self.log("loss/train", loss, on_step=False, on_epoch=True, prog_bar=False,sync_dist=True)
...
class ToSoccerMapTensor:
    ...
    def __call__(self, sample):
        ...
        #진짜 엄청 엄청 어렵게 찾은 오류들...
        #패스 성공, 패스 선택 모델은 모두 target이 패스 성공여부(0 or 1)이지만
        #패스 가치 모델은 train일때는 score_xg를 사용하고, test일때는 scores를 사용하므로
        #아래와 같이 sample['success']를 쓰면 매우 잘못됨
        #int()함수 써버리면, socre_xg값이 모두 0이 되므로 이또한 잘못된 오류
        # target = int(sample["success"]) if "success" in sample else None
        target = sample[self.label] if "success" in sample else None
...
class SoccerMapComponent(PassValueComponent, UnxPassPytorchComponent):
    ...
    def __init__(self, model: PytorchSoccerMapModel, offensive=True, success=True):
        self.value_label = "scores" if offensive else "concedes"
        ...
        super().__init__(
            ...
            label=[self.value_label, f"{self.value_label}_xg", "success"],
            #train학습시에는 xg로 학습하고, test에는 scores로 테스트함
            transform=ToSoccerMapTensor(dim=(68, 104), label=f"{self.value_label}_xg"),
            #transform=soccermap_ToSoccerMapTensor(dim=(68, 104), label=f"{self.label}_xg"),
            
        )

    def initialize_dataset(self, dataset: Callable, transform=None) -> PassesDataset:
        if transform is not None:
            data = dataset(xfns=self.features, yfns=self.label, transform=transform)
        else:
    ...
    def test(self, dataset, config={}) -> Dict:
        ...
        # Load dataset
        data = self.initialize_dataset(dataset, transform=ToSoccerMapTensor(dim=(68, 104), label=f"{self.value_label}"))
