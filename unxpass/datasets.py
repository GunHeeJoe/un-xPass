...
class PassesDataset(Dataset):
    ...
    def __init__(
        ...
        # path: Optional[os.PathbooLike[str]] = None,
    ...
    #dataset.creat(db)수행시 만들어지는 feature의 과정
    #DataBase로부터 액션데이터를 가져오면 filterin & left_to_right과정을 
    #자동으로 수행해서 features & labeling을 수행하는 작업
            ...
            for xfn, _ in self.xfns.items():
                ...
                for game_id in track(
                    ...
                    df_features_xfn.append(
                        ...
                        #모든 action을 left_to_right로 변경하는작업
                        features.get_features(
                            ...
                            #과거 3개의 데이터
                            nb_prev_actions=3,
                            #action type = pass Or cross인 pass
                            #bodypart=foot인 pass
                            # 360visible이 되는 pass 
    ...
    def __getitem__(self, idx: int) -> Dict:
        ...
        if self.transform:
            sample = self.transform(sample, self.train_test)
